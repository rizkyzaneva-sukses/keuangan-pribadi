...
export async function PUT(
  ...
  if (!nama || !Array.isArray(pos) || pos.length === 0) {
    return new NextResponse("Nama & pos required", { status: 400 });
  }

  const total = pos.reduce((sum, item) => sum + Number(item.persentase || 0), 0);
  if (total !== 100) {
    return new NextResponse("Total persentase harus 100%", { status: 400 });
  }

  const names = pos.map((item) => String(item.nama || "").trim()).filter(Boolean);
  if (names.length !== pos.length) {
    return new NextResponse("Semua pos harus memiliki nama", { status: 400 });
  }
  if (new Set(names).size !== names.length) {
    return new NextResponse("Nama pos dalam satu template tidak boleh sama", { status: 400 });
  }
  ...
  for (const item of pos) {
    if (item.id && !existingById.has(Number(item.id))) {
      return new NextResponse("Ada pos yang tidak valid untuk template ini", { status: 400 });
    }
  }
  ...
  if (blocked.length > 0) {
    ...
    return new NextResponse(
      `Pos "${blockedNames}" sudah dipakai dan tidak bisa dihapus. Set persentase 0% jika ingin menghentikan penggunaannya.`,
      { status: 400 }
    );
  }
