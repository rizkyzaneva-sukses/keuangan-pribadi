...
export async function POST(req: NextRequest) {
  ...
  if (!nama || !pos?.length) return new NextResponse("Nama & pos required", { status: 400 });
  const total = pos.reduce((s, p) => s + Number(p.persentase || 0), 0);
  if (total !== 100) return new NextResponse("Total persentase harus 100%", { status: 400 });

  // Jika isDefault, unset default lainnya
  if (isDefault) {
  ...
  // Buat wallet pos untuk pos baru
